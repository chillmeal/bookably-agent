...
2. Fill required secrets (`TG_BOT_TOKEN`, `ACP_API_KEY`, etc.). `LLM_API_KEY` is required for `openrouter` and optional for `stub`.
...
| `BOOKABLY_BOT_SERVICE_KEY` | Yes | - | Shared backend service key for bot server-to-server auth |
| `LLM_PROVIDER` | Yes | - | `openrouter` or `stub` |
| `LLM_API_KEY` | Conditional | - | Required for `openrouter`, optional for `stub` |
